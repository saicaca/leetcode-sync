...
async function displayIcon() {
    ...
    while(true) {
        let btnRun = document.querySelector('[data-cypress="RunCode"]')
        if(btnRun) {
            btnRun.insertAdjacentHTML('beforebegin', iconHtml)
            break
        }
        await sleep(1000)
    }
...
async function setIconStatus(status) {
    let iconElement = document.querySelector('#sync-icon')
    if(status) {
        iconElement.className = status
    }
}
